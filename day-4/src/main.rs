use std::fs::File;
use std::io::{Write, stdin};  // using standard input/output 

//funtion for input
fn input() -> String {
    let mut input1 = String::new();
    stdin().read_line(&mut input1).expect("Not a valid input");
...
fn main() {
    ...
    let columns = vec![
        ("Matric No", "Name", "Department", "Course Code", "First Test", "Second Test", "Participation Mark", "Exam", "Total", "Grade"),
    ];
    ...
    println!("
     ...
     // ASKING FOR OTHER STUDENTS DETAILS
        ...
        let mut file = File::create("simslite.txt").expect("write failed");
        ...
        file.write_all(separator.as_bytes()).expect("write failed");

        for name in &student_list{
            let line = format!("{} {} {} {} {} {} {} {} {} {}", 
            matric_no, name, department, course_code, f_score, s_score, p_mark, exam, total, grade);
            file.write_all(line.as_bytes()).expect("write failed");
  }
...
/* Okay I don't like giving up but I guess I have to do so now, it's supposed to print multiple
student details but it's just printing now, I think I can figure it out later but for now, make
i just submit. */